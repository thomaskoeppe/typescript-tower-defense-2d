...
export class ButtonGroup extends Phaser.GameObjects.Container {
    ...
    constructor (scene: GameScene, {x: x, y: y}: {x: number, y: number}, {w: w, h: h}: {w: number, h: number}, buttons: Button[]) {
        ...
        this.tooltip = this.scene.add.text(0, 0, '', {fontFamily: 'Public Pixel', fontSize: '16px', color: '#ffffff', align: 'center'}).setOrigin(0.5, 0).setDepth(LayerDepth.UI_ITEM).setVisible(false);
